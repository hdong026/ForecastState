"""Final adaptive spatial graph module (G1_final_adaptive)."""
...
class ABCDSpatialModule(nn.Module):
    """Unified spatial module with final adaptive-only refinement."""
    ...
    def __init__(
        ...
        if self.post_spatial_mode not in {"adaptive_multiscale_only"} and self.spatial_scheme in {"A", "B", "C", "D"}:
    ...
    def _init_adaptive_ms_logits(num_scales: int, init_mode: str = "favor_largest") -> torch.Tensor:
        ...
        raise ValueError(f"Unsupported adaptive_ms_init: {init_mode}")
    ...
    def _fuse_adaptive_ms_deltas(self, deltas: list[torch.Tensor]) -> torch.Tensor:
        weights = torch.softmax(self.adaptive_ms_logits, dim=0)
        stacked = torch.stack(deltas, dim=0)
        return torch.einsum("k,kbtn->btn", weights, stacked)
    ...
    def _refine_adaptive_multiscale(self, output: torch.Tensor, history_flow: torch.Tensor) -> torch.Tensor:
        ...
        fused_delta = self._fuse_adaptive_ms_deltas(deltas)
    ...
    def refine_prediction(self, output, history_flow):
        ...
        explicit_modes = {"hybrid", "static_only", "dynamic_only", "static_adaptive", "static_dynamic", "adaptive_dynamic"}
